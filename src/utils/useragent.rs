...
mod tests {
    ...
    fn test_match_user_agent() {
        ...
        // Test Surge with version
        match_user_agent(
            "Surge iOS/version/4.0.0",
            &mut target,
            &mut clash_new_name,
            &mut surge_ver,
        );
        match target {
            SubconverterTarget::Surge(v) => assert_eq!(v, 4),
            _ => panic!("Expected Surge target"),
        }
        assert_eq!(surge_ver, 4);
        ...
        assert_eq!(target, SubconverterTarget::Surge(2));

        // Test unknown user agent
        target = SubconverterTarget::Auto;
        match_user_agent(
            "Unknown Browser",
            &mut target,
            &mut clash_new_name,
