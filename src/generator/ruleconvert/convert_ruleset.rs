...
mod tests {
    ...
    #[test]
    fn test_convert_clash_to_surge() {
        let clash_content = r#"payload:
  - 'domain:example.com'
...
"#;
        ...
        let result = convert_ruleset(clash_content, RulesetType::Surge);
        assert!(result.contains("DOMAIN,example.com"));
        assert!(result.contains("DOMAIN-SUFFIX,example.org"));
        assert!(result.contains("IP-CIDR,8.8.8.8/32"));
        assert!(result.contains("IP-CIDR6,::/0"));
    }

    #[test]
    fn test_convert_quanx_to_surge() {
        let quanx_content = r#"HOST,example.com,PROXY
HOST-SUFFIX,example.org,DIRECT
IP-CIDR,8.8.8.8/32,PROXY,no-resolve
"#;

        let result = convert_ruleset(quanx_content, RulesetType::Surge);
        assert!(result.contains("DOMAIN,example.com"));
        assert!(result.contains("DOMAIN-SUFFIX,example.org"));
        assert!(result.contains("IP-CIDR,8.8.8.8/32,no-resolve"));
    }

    #[test]
    fn test_domain_suffix_with_keyword() {
        let clash_content = r#"payload:
  - '.example.*'
"#;

        let result = convert_ruleset(clash_content, RulesetType::Surge);
        assert!(result.contains("DOMAIN-KEYWORD,example"));
    }

    #[test]
    fn test_plus_dot_domain_suffix() {
        let clash_content = r#"payload:
  - '+.example.com'
"#;

        let result = convert_ruleset(clash_content, RulesetType::Surge);
        assert!(result.contains("DOMAIN-SUFFIX,example.com"));
    }
...
"#;
        ...
        let result = convert_ruleset(clash_content, RulesetType::Surge);
        assert!(result.contains("IP-CIDR6,2001:db8::/32"));
    }
}
