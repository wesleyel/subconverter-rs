...
import { initWasm, SubconverterWasm } from '@/lib/wasm';
...
export async function GET(request: NextRequest) {
    try {
        ...
        if (typeof wasmModule.admin_file_exists !== 'function') {
            throw new Error('admin_file_exists function not available in WASM module');
        }
        ...
        let attributes = null;
        if (wasmModule && typeof wasmModule.admin_get_file_attributes === 'function') {
            try {
